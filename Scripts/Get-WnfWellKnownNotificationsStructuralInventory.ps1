...
.SYNOPSIS
    ...
    Windows well-known Notifications registry key.
...
.DESCRIPTION
    ...
    DataScope, PermanentData, decoded metadata, and Sequence range. This allows
    the observed values to be checked for the expected WellKnown lifetime
    without assuming that every value in the key has the same layout.

    This key is used as the well-known WNF state-name registry store. The script
    does not assume a particular investigation-specific value family here. It
    groups values by registry type, data length, decoded WNF metadata, and full
    decoded state-name fields, and groups readable REG_BINARY payloads by
    SHA-256 hash.
...
.NOTES
    ...
    A missing Control\Notifications key is treated as a valid inventory result:
...
param(
    ...
    [string] $OutputDirectory = (
        Join-Path $env:ProgramData (
            'WindowsWnfRegistryBloatToolkit\WellKnownStructuralInventory'
        )
    ),
...
if (-not ('WnfWellKnownStructuralNative' -as [type])) {
    Add-Type -TypeDefinition @'
using System;
using System.Text;
using System.Runtime.InteropServices;

public static class WnfWellKnownStructuralNative
{
...
$SummaryCsv = Join-Path $OutputDirectory (
    "Wnf-WellKnownStructuralSummary-$SafeLabel-$Timestamp.csv"
)

$StructureGroupsCsv = Join-Path $OutputDirectory (
    "Wnf-WellKnownStructureGroups-$SafeLabel-$Timestamp.csv"
)

$PayloadGroupsCsv = Join-Path $OutputDirectory (
    "Wnf-WellKnownPayloadGroups-$SafeLabel-$Timestamp.csv"
)

$StateNameGroupsCsv = Join-Path $OutputDirectory (
    "Wnf-WellKnownDecodedStateNameGroups-$SafeLabel-$Timestamp.csv"
)
    ...
    if ($null -ne $Key) {
        ...
        for ([uint32] $EnumerationIndex = 0; ; $EnumerationIndex++) {
            if (
                ...
                Write-Progress `
                    -Activity 'Inventorying Control Notifications root values' `
                    -Status (
            ...
            $EnumResult =
                [WnfWellKnownStructuralNative]::RegEnumValue(
                    $Handle,
                    $EnumerationIndex,
                    $NameBuffer,
                    [ref] $NameLength,
            ...
            if (
                $EnumResult -eq
                [WnfWellKnownStructuralNative]::ERROR_NO_MORE_ITEMS
            ) {
                break
            }
            ...
            if (
                $EnumResult -ne
                    [WnfWellKnownStructuralNative]::ERROR_SUCCESS -and
                $EnumResult -ne
                    [WnfWellKnownStructuralNative]::ERROR_MORE_DATA
            ) {
            ...
            if ($ValueType -eq 3) {
                ...
                if (
                    $EnumResult -ne
                    [WnfWellKnownStructuralNative]::ERROR_SUCCESS
                ) {
...
Write-Host
Write-Host 'Well-known Notifications structural inventory completed.'
Write-Host
Write-Host "Server:                      $env:COMPUTERNAME"
