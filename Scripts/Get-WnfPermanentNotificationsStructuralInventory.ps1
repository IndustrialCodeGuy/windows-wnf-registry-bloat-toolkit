...
.DESCRIPTION
    ...
    investigation payload, unique-ID sequence statistics, and decoded WNF
    state-name fields (Version, Lifetime, DataScope, PermanentData, and
    Sequence). The output is intended for comparison between Windows Server
    2019 RDS systems.
...
$StateNameGroupsCsv = Join-Path $OutputDirectory (
    "Wnf-DecodedStateNameGroups-$SafeLabel-$Timestamp.csv"
)
...
try {
    ...
    for ([uint32] $EnumerationIndex = 0; ; $EnumerationIndex++) {
        ...
        try {
            ...
            [uint64] $UniqueId = $WnfFields.Sequence
...
$Summary = [pscustomobject]@{
    ...
    DistinctDecodedStateNameGroups  =
        $StateNameGroups.Count
